...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    width = 350,
      h4("About this tool:"),
      p("In accordance with NHS guidance and GDPR legislation, all organisations in the United Kingdom handling sensitive data have a duty to assess and minimise associated risks. *Host Organisation* handles large volumes of patient data and receives hundreds of requests per year to use this data for research projects. This data risk tool aims to quantify the data risks associated with each project and is informed by guidance from: GDPR, ICO, GMC, DCMS, Five Safes, HIPAA and the NHS. The purpose of this data risk tool is to identify risks associated with each project and ensure that data is only shared where efforts have been made to minimise risks."),
      ...
      HTML("&bull; All studies that use routinely collected patient data from *Host Organisation*, including secondary uses of study data, that do not have specific research consent or a Section 251 consent waiver (which allows the common law duty of confidentiality to be temporarily lifted)."),br(),
      ...
      fluidRow(
        column(12,
               ...
               checkboxGroupInput("q1h", "Q1h: Which of the following best describes your study",
                                  choices = c("Retrospective Study",
                                              "Retrospective Study with Descriptive Statistics Only",
               ...
               radioButtons("q2b", "Q2b: The project has ethical approval and patients consent to researchers accessing their routinely collected data for the purposes of this particular project", choices = c("Yes", "No"), selected = character(0), inline = TRUE),
               conditionalPanel(
               ...
               conditionalPanel(
                 ...
                 HTML('<p style="color:green;">Please continue to the next question.</p>')),
               radioButtons("q2d", "Q2d: The project involves members of the hospital clinical care team who will access identifiable information in order to de-identify the data for analysis", choices = c("Yes", "No"), selected = character(0), inline = TRUE),
               conditionalPanel(
               ...
               conditionalPanel(
                 ...
                 HTML('<p style="color:green;">Please continue to the next question.</p>')),
               radioButtons("q2e", "Q2e: The project involves researchers who have (or will have) a research passport and electronic health record access, but who are not members of the hospital team accessing identifiable information at any stage", choices = c("Yes", "No"), selected = character(0), inline = TRUE),
               conditionalPanel(
                 condition = "input.q2e == 'Yes'",
                 HTML('<p style="color:red;">This project may fall outside the remit of this tool. Please speak to R&D to find a solution.</p>')),
               conditionalPanel(
               ...
               checkboxGroupInput("q3", "Q3: Does your project involve any of the following types of data? Please select all that apply",
                                  choices = c("Biometric data (e.g. fingerprints, retinal scans)" = 1.25,
                                              "Genomic data" = 1.25,
                                              "Linkage with data from outside *Host Organisation*" = 1.25,
                                              ...
                                              "Rare or unusual diagnosis either in general population or *Host Organisation* patients (criteria: UK general population prevalence <1 in 100,000 or <10 new diagnoses at *Host Organisation* per year)" = 1.1,
                                              "Rare or unusual medications or procedures at *Host Organisation* (<10 at *Host Organisation* per year)" = 1.1,
                                              "Outliers in the data set – age, weight, height, length of stay etc" = 1.2,
                                              "Digitised slides or other similar pathology data" = 1,
                                              "Specific timestamps (exact dates and times) relating to individuals, including date of death" = 1.3,
        ...
        checkboxGroupInput("q5", "Q5: How and where will data be used? Please select all that apply",
                           choices = c("Internal study within *Host Organisation* only" = 0.9,
                                       "Study involving transfer to *University Affiliated with Host Organisation*" = 1.0,
                                       "Study with an academic partner organization in the UK" = 1.05,
                                       "Study with public or third sector organisation in the UK, including other NHS trusts" = 1.05,
                                       ...
                                       "Study with an organisation based outside the UK (including academic, public, commercial and third sector)" = 1.2,
        ...
        em(style = "color: blue;", "About Question 5: All relevant options should be selected. If data is stored entirely within a trusted research environment but shared with other partners, then options relating to the sharing with partners should still be included. Answer Question 5a with your best understanding of where the data will be processed. Data leaks are made more likely by the exchange of information between different institutions. Minimising data transfer outside the host institution is one method of safeguarding data security. It is well documented that patients are generally favourable towards their data being shared with NHS institutions and for academic purposes compared with commercial organisations."),
        em(style = "color: blue;", "This does not disqualify such projects from approval, but further precautions may be needed. Similarly, sharing data outside of the country is risky but these risks are somewhat mitigated where the countries have General Data Protection Regulation (GDPR) adequacy decisions. A Trusted Research Environment (TRE) refers to a secure and controlled environment where sensitive and confidential data can be stored, processed, and analyzed for research purposes. If a TRE is being used, it should be ISO 27001 compliant and specific details should be included in Question 7. Please contact R&D if you are unsure about the suitability of your TRE. Please specify if your TRE is NHS-based. The third sector is a term which includes community groups, voluntary organisations, charities, social enterprises and housing associations."),
        hr(),
        checkboxGroupInput("q5a", "Q5a: (If Applicable) For external organisations, where will the partner be processing the data:",
                           choices = c("In the UK" = 1.0,
                                       "Outside the UK but in the EEA" = 1.05,
                                       "Outside the EEA but in a country recognised with GDPR adequacy decision" = 1.05,
        ...
        checkboxGroupInput("q6", "Q6: Who will have access to the de-identified data?",
                           choices = c("Data will only be accessed by members of the clinical care team at *Host Organisation*, other *Host Organisation* staff with a substantive contract or with a non-research contract" = 0.9,
                                       "Data will only be accessed by academic researchers (usually *University Affiliated with Host Organisation* or PPIE) with an honorary research contract or letter of access for *Host Organisation*" = 0.95,
                                       "In addition to or instead of the above options, data will be accessed by staff from another NHS partner organisation, with or without a research passport/honorary contract" = 1.05,
                                       "In addition to or instead of the above options, data will also be accessed by staff from another academic partner organisation, third sector organisation or PPIE members who do not fit into any other category. They may or may not have a research passport/honorary contract" = 1.05,
                                       "In addition to or instead of the above options, data will also be accessed by staff from a commercial partner organisation. They may or may not have a research passport/honorary contract" = 1.1), width="50%"),
        em(style = "color: blue;", "About Question 6: Accompanying notes: Please select as many options as are true. For example, if data will be accessed by clinical *Host Organisation* staff, an NHS partner organisation, another academic partner organisation and a commercial partner organisation then options 3, 4 and 5 should all be selected. PPIE members may not require direct access to the data but if this is being arranged then the access details should be reflected in answering this question. If the deidentified data is accessible to external researchers outside of *Host Organisation or University Affiliated with Host Organisation*, the relevant options should be selected, even if they have a research passport/honorary contract. A research passport is not required for each external researcher accessing deidentified data."),
        hr(),
        checkboxGroupInput("q7", "Q7: What are the security arrangements for data storage? Please select all that apply",
                           choices = c("Within the *Host Organisation* IT environment ONLY, including BYOD" = 0.9,
                                       "Within an area of the *University Affiliated with Host Organisation* covered by an NHS toolkit" = 1.0,
                                       "Within an area of the *University Affiliated with Host Organisation*  not covered by an NHS toolkit" = 1.05,
        ...
        checkboxGroupInput("q8", "Q8: How will the data be transferred between devices?",
                           choices = c("Not applicable" = 0.8,
                                       ...
                                       "Secure email server (e.g. *Host Organisation* email, NHS.net)" = 1.0,
                                       "Standard email" = 10,
        ...
        checkboxGroupInput("q9", "Q9: Public and patient involvement and engagement",
                           choices = c("Patients and public have been involved in the design of this study, including membership on the research team or close involvement in the formulation of the study" = 0.8,
                                       "Patients and public have been consulted on the study but not directly involved in its design [Do not select this option if the above option has already been selected]" = 0.9,
                                       "There is a protocol for the study in the public domain, e.g. as a published article or on a public repository" = 0.95,
                                       "A plain-English summary will be made available online, aimed specifically for public consumption e.g. on a website with an avenue for members of the public to contact the team if needed for further information [This does not include mandatory reporting on data use registries]" = 0.95), width="50%"),
        em(style = "color: blue;", style = "color: blue;", "About Question 9: This question seeks to identify elements of the study design, not captured elsewhere, that can reduce or increase the data risk. Previous review of the study design and involvement of PPI can improve the acceptability of the project. Under normal circumstances a data sharing agreement would be covered under one of the other approvals, but if it has been completed separately then this option should be selected."),
        hr(),
        checkboxGroupInput("q10", "Q10: Other approvals",
                           choices = c("This study has received NHS or University ethics approval" = 0.85,
                                       "There will be a data transfer agreement (DTA) in place for this data transfer/exchange" = 0.95,
                                       "This study has received specific research funding after review by a funding body" = 0.9,
                                       "*Host Organisation* maintains control of the data retention period" = 0.95,
                                       "*Host Organisation* maintains control of data access" = 0.95), width="50%"),
        textAreaInput("q10_DTA", "If there is a relevant data transfer agreement (DTA) for this project, please list it here:", width = "50%"),
        em(style = "color: blue;", "About Question 10: This question seeks to identify elements of the study design, not captured elsewhere, that can reduce or increase the data risk. Previous review of the study design and involvement of PPI can improve the acceptability of the project. Under normal circumstances a data sharing agreement would be covered under one of the other approvals, but if it has been completed separately then this option should be selected."),
        hr(),
        textAreaInput("q11a", "Q11a: Please describe the extent to which patients and the public have been involved in designing this study. Include information about how PPIE members were recruited, specific feedback and changes that have been made as a result of PPIE and plans for ongoing input (max 250 words)", width = "50%"),
